...
export default class SiderMenu extends PureComponent {
  ...
  getNavMenuItems(menusData) {
    ...
    return menusData.map((item) => {
      ...
      const hasSubmMenu = hasPermission && item.children && item.children.some(child => child.name) && !item.children.every(child => child.hideInMenu);
      if (hasSubmMenu) {
        if (item.hideInMenu) {
          return null;
        } else {
      ...
      (
        <div className={styles.logoInner}>
          ...
          <div className={styles.names}>
            <div className={styles.name}>管理系统</div>
            <div className={styles.subName}>dtdream.com</div>
          </div>
        </div>
      );
    return (
